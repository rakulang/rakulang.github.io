...
捕获是用于将参数传递给代码对象的容器。 捕获是link:https://docs.raku.org/type/Signature[签名]的另一面 - 捕获是调用者定义的参数，而签名是被调用者定义的参数。

当你调用 `print $a, $b` 时，`$a ,$b` 部分是一个 Capture。

Captures 包含用于类似列表的部分的位置参数和类似哈希部分的命名参数，因此表现为 link:https://docs.raku.org/type/Positional[位置性]和link:https://docs.raku.org/type/Associative[关联性]，尽管它实际上并不混合这些角色。 对于命名参数，Captures 使用与普通List略有不同的语法。 
...
返回包含所有位置键的 link:https://docs.raku.org/type/Seq[Seq]，后跟所有命名键。对于位置参数，键是从零开始的相应参数序号位置。
...
返回包含所有位置值的 link:https://docs.raku.org/type/Seq[Seq]，后跟所有命名参数值。
...
返回交替键和值的 link:https://docs.raku.org/type/Seq[Seq]。首先是位置link:https://docs.raku.org/type/Capture#method_keys[键]和link:https://docs.raku.org/type/Capture#method_values[值]（如果有），然后是命名键和值。
...
返回所有参数，即命名后跟的位置，作为已交换键和值的对的Seq，即值变为键，键变为值。此行为与 link:https://docs.raku.org/type/Capture#method_pairs[pair] 方法相反。
