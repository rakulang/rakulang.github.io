...
这先匹配一个引号, 然后匹配任何不是引号的字符, 再然后还是一个引号。 上面例子中的 `*` 和 `+` 会在 link:https://docs.raku.org/language/regexes#Quantifiers[量词]一节中解释。
...
一个非量词化的捕获产生一个 link:https://docs.raku.org/type/Match[Match] 对象。当捕获被量词化(除了使用 `?` 量词)之后, 该捕获就变成 link:https://docs.raku.org/type/Match[Match] 对象的列表。
...
命名正则可以规整到 link:https://docs.raku.org/language/grammars[gramamrs] 中。link:https://design.raku.org/S05.html#Predefined_Subrules)[S05-regex] 中有一组预定义的 subrules。
