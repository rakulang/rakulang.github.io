...
因为 `=>` 构造了一个 link:http://doc.raku.org/type/Pair[Pair]，它有一个 `key` 和一个 `value` 属性。子签名中的 `:$key` 命名参数提取属性 `key`。 `\*%` 中的 `%` 是匿名的, `*%` 接收所有剩余参数到一个匿名散列中, 不予使用。
...
通常, 对象是基于对象的属性来解构的. 惯用法就是在 for 循环中解包一个 link:https://docs.raku.org/type/Pair[Pair] 的键和值:
...
.timicles.pl6
----
raku timicles.pl6 1d 2h 3m 5s
----

对象解包为它们的属性只是默认行为. 要使对象以另外的方式解构, 要更改它的 link:https://docs.raku.org/routine/Capture[Capture].
